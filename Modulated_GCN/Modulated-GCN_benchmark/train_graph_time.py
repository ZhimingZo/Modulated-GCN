...
def step(split, opt, actions, dataLoader, model, criterion, optimizer=None):#, key_index=None
    ...
    model_module_gcn = model['module_gcn']
    model_post_refine = model['post_refine']


    if split == 'train':
        model_module_gcn.train()
        if opt.out_all:
            out_all_frame = True
        else:
            out_all_frame = False
    ...
    for i, data in enumerate(tqdm(dataLoader, 0)):
        ...
        if opt.test_augmentation and split =='test':
            input_2D, output_3D = input_augmentation(input_2D, model_module_gcn, joints_left, joints_right)
        else:
            input_2D = input_2D.view(N, -1, opt.n_joints,opt.in_channels, 1).permute(0, 3, 1, 2, 4).type(torch.cuda.FloatTensor) # N, C, T, J, M
            output_3D = model_module_gcn(input_2D)
...
def input_augmentation(input_2D, model_module_gcn, joints_left, joints_right):
    """
    for calculating augmentation results
    :param input_2D:
    :param model_module_gcn:
    ...
    output_3D_flip = model_module_gcn(input_2D_flip)
    output_3D_flip[:, 0] *= -1
    output_3D_flip[:, :, :, joints_left + joints_right] = output_3D_flip[:, :, :, joints_right + joints_left]
    output_3D_non_flip = model_module_gcn(input_2D_non_flip)
